...
- **🗣️ Voz neuronal local de Jimmy** (botón 🗣️ o comando `piper`): Jimmy **habla los briefings y las lecciones** con voz neuronal Piper 100 % local y gratis (el servidor de Aion Sincro, `127.0.0.1:8766`) — sin internet ni claves. Detecta el servidor solo, permite elegir voz y degrada en silencio si no está activo.
...
| Sistema | `ayuda`, `tutorial`, `explicar`, `preguntar`/`jimmy`, `reto`, `ranking`, `examen`, `sonido`, `piper`, `habilidades`, `demo`, `clear`, `history`, `carrera`, `glosario`, `whoami`, `date` |

> 💡 `Tab` autocompleta · ↑/↓ historial · Ctrl+L limpia.

## 🗣️ Voz local de Jimmy — Piper TTS

Jimmy puede **hablar de verdad** sus briefings y lecciones con voz neuronal en español, 100 % local y gratuita, reutilizando el **servidor Piper de Aion Sincro** (`piper_server.py`, que escucha en `http://127.0.0.1:8766` con las mismas protecciones: solo loopback y Origin validado).

**Cómo activarla:**
1. Arranca el servidor (una sola vez): `windows/instalar-piper.cmd` (Windows) o `linux/instalar-piper.sh` (Linux/macOS) — crea el venv, instala `piper-tts`, descarga la voz `es_ES-sharvard-medium` y deja el servidor escuchando. O manual: `.venv-piper\Scripts\python piper_server.py`.
2. En el juego: pulsa el botón **🗣️** de la barra superior o escribe `piper on`.
3. `piper estado` te dice si el servidor responde y qué voces hay instaladas; `piper voz <slug>` cambia de voz (p. ej. `es_ES-davefx-medium`) y `piper url <url>` / `piper token <clave>` configuran un servidor o token distintos.

**Notas:** la voz está **desactivada por defecto** (sin ella el juego funciona igual, en silencio) y solo funciona cuando el juego se sirve en local (`node serve.js` o `CYBERGRAD.bat`) — en GitHub Pages no hay servidor local, así que degrada sin romper nada. El texto se limpia de markdown antes de sintetizar y las frases largas se parten en fragmentos.
...
> ⚖️ Todo ocurre en un entorno ficticio autorizado (CiberCorp). Las técnicas se explican para aprender a **defender** y a **probar** sistemas propios.
...
1. Crea `js/casos/caso-NN-nombre.js` siguiendo la estructura de los existentes:
   ...
   - `incorrectas` — errores típicos a penalizar (`"bloquear|dominio:cibercorp.com"`)
...
- **`checks`** — la cadena completa de **21 tests** (`npm test`):
  ...
  7. **Whoami E2E** (`test:whoami`) — el comando `whoami` distingue "del SOC de CiberCorp" (blue team) de "de la Unidad Red Team de CiberCorp" (pentests).
  ...
  22. **`_headers` en host compatible** (`test:headers`) — como GitHub Pages ignora `_headers`, este test levanta un **CDN local** (`ci/headers-server.mjs`) que lee `_headers` con la sintaxis Netlify/Cloudflare Pages (bloques de ruta + cabeceras indentadas) y verifica con HTTP real que la **CSP completa llega como cabecera** — incluido `frame-ancestors 'none'` (que por `<meta>` el navegador ignoraría) — junto a `X-Frame-Options: DENY`, `nosniff` y `Referrer-Policy`, tanto en `/` como en `/sw.js`. 17 checks: parser, matcher de rutas y E2E del CDN local.
  23. **Easter egg del nombre** (`test:easteregg`) — el comando oculto `porque_cybergrad` explica en la terminal que *grad* (град) significa «ciudad» en eslavo (CYBERGRAD = «la ciudad del ciber»), **no aparece en `ayuda`** (easter egg de verdad: se descubre por casualidad o por el README), y Jimmy lo explica también por lenguaje natural (`preguntar porque se llama cybergrad` / «¿qué significa grad?») tanto sin caso activo como con caso — sin falsos positivos en otras preguntas. 10 checks: lógica pura de Jimmy + E2E en la terminal sin errores de consola.
  24. **Banner del README** (`test:readme-banner`) — el banner ASCII que muestra el README (el mismo de la terminal) **coincide EXACTO fila a fila con el BANNER de `js/main.js`** (extraído con el mismo `banner-core.mjs` del golden) y además es **canónico** (glifos figlet Standard, letra a letra, sin glifos compartidos). Con **prueba de mutación**: si alguien cambia un glifo en el README o en `main.js` sin actualizar el otro, el test lo caza. 18 checks en Node puro.
- **`integracion`** — check de integración real: `test:prod` carga la **versión desplegada en GitHub Pages** con Playwright y verifica **69 checks** contra los mismos canónicos de los tests locales: el **banner** (golden + glifos, vía `banner-core.mjs`), el **subtítulo**, el **pie de arranque** y el **separador ASCII** (vía `visual-core.mjs`, completando el onboarding y ejecutando `ayuda` para generar un separador real), los **metadatos** (vía `meta-core.mjs`: título de pestaña, Open Graph y Twitter Card leídos del DOM, con el **invariante relacional** de que el `og:title` desplegado coincide EXACTAMENTE con el título de pestaña servido — además de ambos contra el canónico —, el **JSON-LD SoftwareApplication** leído del DOM desplegado (snippet rico de Google) verificado contra el mismo canónico, las **og:image comprobadas con HTTP real** — un archivo roto o desaparecido rompería la tarjeta de LinkedIn — y la **meta CSP** con sus directivas clave y la garantía de que NO lleva `frame-ancestors`, que el navegador ignora por `<meta>`) y las **cabeceras HTTP reales** que sirve producción (leídas con fetch: GitHub Pages **no sirve la CSP como cabecera** — ignora `_headers` — así que se verifica que la meta es la política efectiva, que **HSTS está presente** y que la ausencia de `X-Frame-Options` queda cubierta por `frame-ancestors 'none'` de `_headers` para hosts que sí lo apliquen) y la **PWA** (manifest instalable servido como `manifest+json`, iconos 192/512 con HTTP real y `sw.js` con su precache y estrategia offline). Además, el job verifica la **página del repo en GitHub** (`test:readme-prod`): vía la API con `Accept: application/vnd.github.html` (el MISMO pipeline de render de GitHub, con `GITHUB_TOKEN` para el rate limit) comprueba que el **banner ASCII aparece en el README renderizado**, coincide **EXACTO** con el de `js/main.js`, es canónico (glifos figlet Standard, letra a letra) y que el blockquote de la etimología («grad» = «ciudad») está presente — con prueba de mutación. 18 checks contra la versión desplegada del repo. Y su **vista móvil** (`test:readme-mobile`, 18 checks): carga el mismo render a **375×812** (iPhone) con el CSS oficial de GitHub y verifica que el banner NO se descuadra — `white-space: pre` (las líneas nunca se parten), `overflow-x: auto` con scroll horizontal (`scrollWidth > clientWidth`) y las 6 filas canónicas intactas, sin errores de página. Solo corre en push a `main` (y manual vía `workflow_dispatch`), reintentando hasta 5 min por si el deploy de Pages está en curso. El badge **«Verificación de producción · 69 checks»** de la cabecera refleja el estado de este job (pasando/fallando) y enlaza al último run del job `integracion`.
...
Uso **exclusivamente educativo**. Empresas, personas, redes e indicadores son ficticios (CiberCorp y su infraestructura no existen).
