...
export function diagnosticar(filas) {
  ...
  if (realNorm.length === 6) {
    ...
    // Regresión real cazada por el usuario: la C se dibujaba con el glifo de
    // la G (GLIFOS.C === GLIFOS.G y el banner idéntico). Dos letras distintas
    // NUNCA deben compartir glifo; se comparan las letras únicas de PALABRA.
    const unicas = [...new Set(PALABRA)];
    const duplicados = [];
    for (let i = 0; i < unicas.length; i++) {
      for (let j = i + 1; j < unicas.length; j++) {
        const a = unicas[i], b = unicas[j];
        if (GLIFOS[a].every((r, f) => r.trim() === GLIFOS[b][f].trim())) {
          duplicados.push(`${a} y ${b}`);
        }
      }
    }
    checks.push({
      nombre: "ninguna letra comparte glifo con otra",
      ok: duplicados.length === 0,
      extra: duplicados.length ? `(glifos idénticos: ${duplicados.join(", ")})` : "",
    });
  }
